...
fn main() {
    ...
    let named_entities: HashMap<&str, &str> = named_entitites_json.as_object().unwrap().iter().map(|(name, v)| {
        let chars = v.as_object().unwrap().get("characters").unwrap().as_str().unwrap();
        (name.as_str(), chars)
    }).collect();
    ...
    for (name, chars) in named_entities {
        ...
        named_entities_rs.push_str(&format!("    (\"{}\".to_string(), \"{}\".to_string()),\n", name, chars.escape_unicode()))
    }
    named_entities_rs.push_str(
    "])
    }");
